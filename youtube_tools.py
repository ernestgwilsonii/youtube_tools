...
def download_video(url, output_path=None, format=None, extract_audio=False, audio_format="mp3", audio_quality="192"):
    """
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            if os.path.exists(mp3_filename):
                ...
                if os.path.exists(alt_filename):
                    ...
                    logger.error(f"Could not find downloaded audio file. Expected at: {mp3_filename} or {alt_filename}")
                    return None
    except Exception as e:
        logger.error(f"Error downloading audio: {e}")
        return None

def convert_format(input_path, output_path=None, to_format="mp4", audio_only=False, quality="192"):
    """
    ...
    if output_path is None:
        ...
        output_path = os.path.join(downloads_dir, f"{base_name}_converted.{to_format}")
    ...
    if audio_only:
        ...
        if to_format in ['mp3', 'aac', 'wav', 'ogg', 'flac', 'm4a']:
            ...
            logger.warning(f"Unsupported audio format: {to_format}, defaulting to mp3")
            to_format = 'mp3'
            cmd.extend(['-q:a', quality])
        ...
        if to_format in ['mp4', 'mkv', 'webm', 'avi', 'mov']:
            ...
            if to_format == 'mp4':
                cmd.extend(['-c:v', 'libx264', '-crf', '23', '-c:a', 'aac', '-b:a', f'{quality}k'])
            elif to_format == 'webm':
                cmd.extend(['-c:v', 'libvpx-vp9', '-crf', '30', '-b:v', '0', '-c:a', 'libopus', '-b:a', f'{quality}k'])
            elif to_format == 'mkv':
                cmd.extend(['-c:v', 'libx264', '-crf', '23', '-c:a', 'aac', '-b:a', f'{quality}k'])
            elif to_format == 'avi':
                cmd.extend(['-c:v', 'mpeg4', '-q:v', '6', '-c:a', 'libmp3lame', '-q:a', '4'])
            elif to_format == 'mov':
                cmd.extend(['-c:v', 'libx264', '-crf', '23', '-c:a', 'aac', '-b:a', f'{quality}k'])
        else:
            logger.warning(f"Unsupported video format: {to_format}, defaulting to mp4")
            to_format = 'mp4'
            cmd.extend(['-c:v', 'libx264', '-crf', '23', '-c:a', 'aac', '-b:a', f'{quality}k'])
...
def download_youtube_transcript(url, output_text_path=None, languages=None):
    """
...
def transcribe_audio(audio_path, output_text_path=None, chunk_size_minutes=2):
    """
...
def download_and_transcribe(url, audio_output=None, text_output=None, chunk_size_minutes=2, 
                           use_youtube_transcript=True, languages=None):
    """
...
def compare_transcripts(transcript1_path, transcript2_path, output_path=None):
    """
    ...
    try:
        ...
        if output_path is None:
            ...
            output_path = os.path.join(downloads_dir, f"{base_name1}_vs_{base_name2}_comparison.txt")
...
def main():
    ...
    download_parser.add_argument('--output', '-o', help='Output path (default: downloads/%(title)s.%(ext)s)')
    download_parser.add_argument('--format', '-f', help='Format to download (e.g., "bestvideo[height<=720]+bestaudio/best[height<=720]")')
    
    # Download-audio command
    download_audio_parser = subparsers.add_parser('download-audio', help='Download a video\'s audio')
    download_audio_parser.add_argument('url', help='URL of the video to download')
    download_audio_parser.add_argument('--output', '-o', help='Output path (default: downloads/%(title)s.mp3)')
    ...
    convert_parser.add_argument('--output', '-o', help='Output path (default: [input_path]_converted.ext)')
    convert_parser.add_argument('--to-format', '-t', required=True, 
                              help='Target format (e.g., mp4, mkv, webm, mp3, aac, wav, etc.)')
    convert_parser.add_argument('--audio-only', '-a', action='store_true',
                              help='Extract audio only')
    convert_parser.add_argument('--quality', '-q', default='192',
                              help='Audio quality for audio conversion (default: 192)')
    
    # Download-transcript command
    download_transcript_parser = subparsers.add_parser('download-transcript', 
    ...
    download_transcript_parser.add_argument('--output', '-o', help='Output path for the transcript')
    download_transcript_parser.add_argument('--languages', '-l', nargs='+', default=['en'], 
    ...
    transcribe_parser.add_argument('--output', '-o', help='Output path for the transcript (default: [audio_path]_transcript.txt)')
    transcribe_parser.add_argument('--chunk-size', '-c', type=int, default=2, 
                                 help='Size of audio chunks in minutes for processing (default: 2)')
    
    # Compare transcripts command
    compare_parser = subparsers.add_parser('compare-transcripts', 
    ...
    compare_parser.add_argument('--output', '-o', help='Output path for the comparison results')
    
    # Download and transcribe command
    download_transcribe_parser = subparsers.add_parser('download-and-transcribe', 
    ...
    download_transcribe_parser.add_argument('--audio-output', '-a', help='Output path for the audio')
    download_transcribe_parser.add_argument('--text-output', '-t', help='Output path for the transcript')
    download_transcribe_parser.add_argument('--chunk-size', '-c', type=int, default=2, 
                                          help='Size of audio chunks in minutes for processing (default: 2)')
    download_transcribe_parser.add_argument('--force-custom', '-f', action='store_true',
                                          help='Force using custom transcription even for YouTube videos')
    download_transcribe_parser.add_argument('--languages', '-l', nargs='+', default=['en'], 
    ...
    if args.command == 'download':
        ...
        if transcript:
            print(f"\nYouTube transcript downloaded successfully to: {output_path}")
            print(f"Transcript preview (first 150 chars): {transcript[:150]}...")
        else:
            print("\nFailed to download YouTube transcript. The video may not have subtitles available.")
    
    elif args.command == 'compare-transcripts':
        output_path = compare_transcripts(args.transcript1, args.transcript2, args.output)
        if output_path:
            print(f"\nTranscript comparison saved to: {output_path}")
        else:
            print("\nFailed to compare transcripts.")
        ...
        if transcript_path and transcript:
            print("\nDownload and transcription complete!")
            if audio_path:
                print(f"- Audio saved to: {audio_path}")
            print(f"- Transcript saved to: {transcript_path}")
            print(f"- Transcript source: {source.capitalize()} ({source})")
